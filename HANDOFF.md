...
- [ ] Apply the VIA patch: `git am patches/0001-VIA-skin-premium-night-HUD-tokens-type-maneuver-tile.patch` on a fresh branch off `main`. It's verified with plain `git am` once PR #8 is merged. It adds `docs/DESIGN.md` (the style contract) plus tokens, type, a copper accent, the maneuver tile and a US limit sign. It's CSS/markup only and doesn't fix the 13 TS errors.

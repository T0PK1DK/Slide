...
- 2026-09-23 Cursor: `npm run build` clean on `claude/via-skin` — restored typed `recents`/`home`/`work`/`coachDismissed` on `GarageConfig` (with `sub` + safe localStorage merge) and pointed the 4-arg route call at `requestFastRoute`; `0001-VIA-skin*.patch` / `docs/DESIGN.md` were not in the workspace.
